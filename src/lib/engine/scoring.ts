...
function scoreMetric(value: number, b: Benchmark): number {
  const raw =
    b.direction === "higher"
...
const FINANCIAL_BENCHMARKS: Benchmark[] = [
  {
    excellent: 20,
    moderate: 7.5,
    poor: 0,
...
const EMPLOYEE_BENCHMARKS: Benchmark[] = [
  {
    excellent: 85,
    moderate: 70,
    poor: 55,
...
const PROCESS_BENCHMARKS: Benchmark[] = [
  {
    excellent: 30,
    moderate: 45,
    poor: 60,
...
const LEARNING_BENCHMARKS: Benchmark[] = [
  {
    excellent: 20,
    moderate: 10,
    poor: 5,
...
function perspectiveScore(
  ...
  return benchmarks.reduce(
    (sum, b, i) => sum + scoreMetric(values[i], b),
    0
  );
}
