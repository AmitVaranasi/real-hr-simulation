  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-[var(--portal-sidebar-border)] bg-white p-4">
        <h2 className="font-semibold text-[var(--portal-title)]">Active engine parameters</h2>
        <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
          <dt className="text-[var(--portal-muted)]">Discretionary budget</dt>
          <dd className="font-medium">${budget.toLocaleString()}</dd>
          <dt className="text-[var(--portal-muted)]">Industries configured</dt>
          <dd>{Object.keys(INDUSTRY_CONFIGS).length}</dd>
          <dt className="text-[var(--portal-muted)]">BSC benchmarks</dt>
          <dd>{SCORING_BENCHMARKS.length} metrics</dd>
        </dl>
        {effective?.economy_multipliers && (
          <div className="mt-4">
            <h3 className="text-sm font-medium text-[var(--portal-ink)]">Economy multipliers</h3>
      ...
      {scenarioTrace && (
        <section className="rounded-xl border border-[var(--portal-primary)]/30 bg-[var(--portal-primary-soft)]/50 p-4">
